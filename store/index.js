...
const bind = (ev) => {
    ev.on('chats.set', ({ chats: newChats, isLatest }) => {
        ...
        console.debug({ chatsAdded }, 'synced chats')
    })
    ...
    ev.on('contacts.set', ({ contacts: newContacts }) => {
        ...
        console.debug({ deletedContacts: oldContacts.size, newContacts }, 'synced contacts')
    })
    ...
    ev.on('contacts.update', (updates) => {
        for (const update of updates) {
            if (contacts[update.id]) {
                ...
                console.debug({ update }, 'got update for non-existant contact')
            }
        }
    })
    ...
    ev.on('chats.update', (updates) => {
        for (let update of updates) {
            ...
            if (!result) {
                console.debug({ update }, 'got update for non-existant chat')
            }
        }
    })
...
const readFromFile = (path) => {
    ...
    if (existsSync(path)) {
        console.debug({ path }, 'reading from file')
